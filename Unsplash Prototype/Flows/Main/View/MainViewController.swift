...
final class MainViewController: UIViewController, MainViewControllerProtocol {
        ...
        viewModel?.errorPublisher
            .sink { [weak self] error in
                if let error = error as? ErrorModel {
                    handleError(error, in: self ?? UIViewController())
                }
            }
            .store(in: &cancellables)
        
        viewModel?.showError = { [weak self] error in
            handleError(.fetchingFailure, in: self ?? UIViewController())
        }
        
        viewModel?.showNoResultsPlaceholder = { [weak self] in
            self?.showPlaceholder()
            handleError(.noData, in: self ?? UIViewController())
        }
        
        viewModel?.resetPlaceholderState = { [weak self] in
            self?.resetPlaceholderState()
        }
    }
    ...
    func searchButtonTapped() {
        ...
        viewModel?.resetPlaceholderState?()
        activateLoader()
...
extension MainViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        viewModel?.showDetailView(for: selectedImage)
    }
}
...
extension MainViewController {
    ...
    private func resetPlaceholderState() {
        placeholderStackView.isHidden = true
        collectionView.isHidden = true
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if offsetY > contentHeight - scrollViewHeight * 1.5 {
            ...
            viewModel?.loadAnotherPage(query: query)
        }
    }
}
...
#Preview {
    ...
    let mainViewCoordinator = MainViewCoordinator(navigationController: UINavigationController(), appCoordinator: appCoordinator)
    let testModel = MainViewModel(coordinator: mainViewCoordinator)
